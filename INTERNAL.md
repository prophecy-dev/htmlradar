...
# recipient worker
npx wrangler deploy --var SHARE_HOST:<host> --var APP_ORIGIN:<dashboard origin> --var MAIL_FROM:<addr>
printf %s "$HTMLRADAR_SESSION_SECRET" | npx wrangler secret put SESSION_SECRET
# dashboard: next-on-pages does not build on native Windows; build in WSL/Linux/CI
cd packages/app && NEXT_PUBLIC_SHARE_ORIGIN=<share origin> pnpm build:edge
