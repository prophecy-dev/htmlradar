-- 025_notify_title_link.sql
-- ------------------------------------------------------------
-- Make the document title in the first-open email a clickable link.
--
-- Until now the email had exactly one working link: the "See the read →"
-- button. But the big italic document title IS the visual hero of the
-- email — people instinctively click it, and nothing happened. This
-- wraps the title in an <a> pointing at the SAME per-share dashboard as
-- the button (color:inherit, no underline — the title looks identical,
-- it's just clickable now). The button stays as the explicit CTA.
--
-- DEPENDENCY: the link target is https://htmlradar.com/dashboard/<slug>.
-- That URL only resolves to the per-share analytics page once the
-- /dashboard/:slug → /docs redirect is removed from the app
-- (next.config.mjs). Apply this AFTER that change is live, or the title
-- link (like the button) bounces to the library.
--
-- Supersedes the email template from 021. Diff vs 021 is exactly two
-- things, everything else (per-document dedup, internal-viewer guard,
-- Vault lookup, timezone rendering, pg_net POST, notifications_log) is
-- byte-for-byte identical:
--   1. headline <p> content wrapped in <a href="%s">…</a>
--   2. one extra v_dashboard_url arg inserted into format() at the
--      headline position (the CTA's v_dashboard_url arg is unchanged)
--
-- Apply: paste into the Supabase SQL editor, run once. Idempotent
-- (`create or replace`). Confirm by opening a brand-new share as a
-- never-seen recipient and clicking the doc title in the email — it
-- should land on /dashboard/<slug>, same as the button.
-- ------------------------------------------------------------
...
begin
  ...
  -- Body template carried from 021. ONLY change: the doc-title headline
  -- is now wrapped in an <a> to v_dashboard_url so the title itself is
  -- clickable (color:inherit, no underline — visually unchanged). The
  -- "See the read →" button below remains the explicit primary CTA.
  v_body := format($html$
<!doctype html>
<html lang="en">
<head>
