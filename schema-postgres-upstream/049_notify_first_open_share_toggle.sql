-- 049_notify_first_open_share_toggle.sql
-- ------------------------------------------------------------
-- Per-share switch for the first-open email:
--   document_shares.config->>'notify_first_open'
-- Default stays on (absent or anything but the string 'false'). Set it to
-- false on the public demo share so a launch-day burst of anonymous opens
-- does not send one email per open to the share owner and exhaust the
-- Resend daily quota that onboarding and customer first-open alerts share.
--
-- Function body is 025_notify_title_link.sql plus the one guard marked
-- "049" directly after the four row lookups. Everything else is
-- byte-for-byte identical to 025.
--
-- Apply: paste into the Supabase SQL editor, run once. Idempotent
-- (`create or replace`). Then flag the demo share:
--   update document_shares
--   set config = config || '{"notify_first_open": false}'::jsonb
--   where slug = 'lumenforge-demo';
-- Confirm by opening the demo as a never-seen recipient: notifications_log
-- gets a 'skipped' row saying "first-open email disabled on this share".
-- ------------------------------------------------------------
